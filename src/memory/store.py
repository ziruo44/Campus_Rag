...
from memory.compaction import refresh_thread_context_summary
from memory.locks import locked_path
...
class ThreadStore:
    ...
    def update_thread(
        ...
        with measure_stage("memory.update_thread"):
            with self._thread_lock(thread_id):
                ...
                previous_compacted_turn_count = document.context_compacted_turn_count
                if not preserve_notices:
                    document.notices = []
                updater(document)
                refresh_thread_context_summary(
                    document,
                    notice_callback=self.settings.compaction_notice_callback,
                )
                if not preserve_notices:
                    self._record_compaction_notice(
                        document,
                        previous_compacted_turn_count=previous_compacted_turn_count,
                    )
    ...
    def _read_document(self, path: Path, thread_id: str) -> ThreadDocument:
        ...
        refresh_thread_context_summary(document)
        if (
            migrated
            or payload.get("version") != THREAD_DOCUMENT_VERSION
            or payload.get("context_summary") != document.context_summary
            or payload.get("context_compacted_turn_count") != document.context_compacted_turn_count
            or payload.get("context_summary_updated_at") != document.context_summary_updated_at
        ):
            self._write_document(path, document)
        return document
    ...
    def _truncate_summary(self, summary: str) -> str:
        ...
        if limit == 0 or len(summary) <= limit:
            return summary
        return truncate_text(summary, limit=limit)

    def _record_compaction_notice(
        self,
        document: ThreadDocument,
        *,
        previous_compacted_turn_count: int,
    ) -> None:
        compacted_delta = document.context_compacted_turn_count - previous_compacted_turn_count
        ...
        document.notices.append(
            {
                ...
                "message": (
                    f"Older conversation context was compacted to keep the active prompt focused. "
                    f"{compacted_delta} turn(s) were merged into memory summary."
                ),
                "compacted_turn_count": document.context_compacted_turn_count,
